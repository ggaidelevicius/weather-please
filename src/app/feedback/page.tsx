...
const Page: FC<{
	...
	const locale =
		Array.isArray(searchParams?.locale) && searchParams.locale.length > 0
			? searchParams.locale[0]
			: searchParams.locale ?? 'en'
	const type =
		Array.isArray(searchParams?.type) && searchParams.type.length > 0
			? searchParams.type[0]
			: searchParams.type ?? 'feedback'
	const installed =
		Array.isArray(searchParams?.installed) && searchParams.installed.length > 0
			? searchParams.installed[0]
			: searchParams.installed ?? 0
	...
	useEffect(() => {
		changeLocalisation(locale as string, true)
