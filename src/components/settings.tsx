      ...
      >
        ...
        <div className="fixed inset-0 flex w-screen items-center justify-center overflow-y-auto p-4">
          ...
          >
            ...
            />
            <Switch label={(<Trans>Use keyboard shortcuts</Trans>) as unknown as string} checked={input.useShortcuts} onChange={(e) => handleChange('useShortcuts', e)} />
            <Input
            ...
            />
            <Switch label={(<Trans>Use metric number format</Trans>) as unknown as string} checked={input.useMetric} onChange={(e) => handleChange('useMetric', e)} />
            <Switch label={(<Trans>Show weather alerts</Trans>) as unknown as string} checked={input.showAlerts} onChange={(e) => handleChange('showAlerts', e)} />
            {input.showAlerts &&
              <>
                <Switch label={(<Trans>Show extreme UV alerts</Trans>) as unknown as string} checked={input.showUvAlerts} onChange={(e) => handleChange('showUvAlerts', e)} />
                <Switch label={(<Trans>Show strong wind alerts</Trans>) as unknown as string} checked={input.showWindAlerts} onChange={(e) => handleChange('showWindAlerts', e)} />
                <Switch label={(<Trans>Show low visibility alerts</Trans>) as unknown as string} checked={input.showVisibilityAlerts} onChange={(e) => handleChange('showVisibilityAlerts', e)} />
              </>
            }
            <h2 className="mt-10 text-2xl font-medium text-white">
