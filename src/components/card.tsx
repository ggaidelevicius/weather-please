...
const Card = ({ day, max, min, description, wind, rain, uv, useMetric, identifier }: CardProps) => {
  const displayedIdentifier = identifier === 'day' ? days[new Date(day * 1000).getDay()] : `${new Date(day * 1000).getDate()} ${months[new Date(day * 1000).getMonth()]}`

  return (
    <div className="bg-dark-700 rounded-md p-5">
      <span className="text-white font-bold text-2xl">{displayedIdentifier}</span>
      <div className="flex justify-between mt-1.5 items-center gap-4">
        <div className="flex flex-col">
          <div className="flex gap-2 items-baseline">
            <span className="text-4xl text-white">{max}</span>
            <span className="text-xl text-dark-300">{min}</span>
          </div>
          <span className="text-dark-100">{descriptionMap[description as keyof typeof descriptionMap]}</span>
        </div>
        <Image
          quality={100}
          priority
      ...
      <div className="flex flex-row gap-4 mt-4 justify-between">
        <div className="flex flex-row items-center gap-2">
          ...
          <span aria-hidden className="text-dark-100">{Math.round(uv)}</span>
        ...
        <div className="flex flex-row items-center gap-2">
          ...
          <span aria-hidden className="text-dark-100">{`${Math.round(rain)}%`}</span>
          <span className="sr-only">
