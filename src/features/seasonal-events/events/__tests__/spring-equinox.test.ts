...
describe('Spring equinox scene', () => {
	it('keeps a static scene visible on resize and rebuilds cached artwork only when pixel density changes', async () => {
		const artworkFactories = [
			vi.spyOn(branchArtwork, 'createSpringBranchArtwork'),
			vi.spyOn(meadowArtwork, 'createSpringMeadowArtwork'),
		]
		const scene = await createScene({ isReducedMotion: true })
		const canvas = document.querySelector('canvas[data-spring-equinox]')

		expect(canvas).toHaveAttribute('data-spring-equinox', 'true')
		...
		for (const factory of artworkFactories) {
		...
		for (const factory of artworkFactories) {
			...
			expect(factory).toHaveBeenLastCalledWith({ dpr: 2 })
		}
		expect(scene.pending.size).toBe(0)
	})
	...
	it.each(['running', 'queued', 'resumed'])(
		...
		async (frameState) => {
			...
			expect(document.querySelector('[data-spring-equinox]')).toBeNull()
			expect(scene.pending.size).toBe(0)
	...
	it('keeps one scene and one animation loop after repeated cleanup and restart', async () => {
		...
		for (let run = 0; run < 3; run += 1) {
			expect(document.querySelectorAll('[data-spring-equinox]')).toHaveLength(1)
			expect(document.body.querySelectorAll('canvas')).toHaveLength(1)
			expect(scene.pending.size).toBe(1)
			cleanupEffect()
			expect(document.querySelectorAll('[data-spring-equinox]')).toHaveLength(0)
			expect(scene.pending.size).toBe(0)
			if (run < 2) cleanupEffect = await launchSpringEquinoxGrowth()
		}
	})
})
...
const createScene = async ({
	...
	cleanupEffect = await launchSpringEquinoxGrowth()
