...
describe('Earth Day scene', () => {
	it('waits for its delayed mount and can be canceled before any scene appears', async () => {
		...
		expect(document.querySelector('[data-earth-day]')).toBeNull()
	...
	it('keeps all four shape families in the static scene and reuses artwork across size and pixel-density changes', async () => {
		const createArtwork = vi.spyOn(artwork, 'createEarthDayArtwork')
		const scene = await createScene({ isReducedMotion: true })
		const canvas = document.querySelector('canvas[data-earth-day]')

		expect(canvas).toHaveAttribute('data-earth-day', 'true')
		...
		expect(document.querySelectorAll('[data-earth-day]')).toHaveLength(1)
		...
		if (!cachedArtwork || cachedArtwork.type !== 'return') {
			throw new Error('Expected cached Earth Day artwork')
		}
		const paintedSources = new Set(
			scene.context.drawImage.mock.calls.map(([source]) => source),
		)
		for (const kind of ['leaf', 'sprout', 'drop', 'flower'] as const) {
			expect(
				cachedArtwork.value.sprites[kind].some((sprite) =>
					paintedSources.has(sprite),
				),
			).toBe(true)
		}
	...
	it('ignores canceled callbacks after pause and resume without starting an extra loop', async () => {
		...
		if (!staleCallback) throw new Error('Expected a scheduled Earth Day frame')
	...
	it('cleans up roots and any listeners if artwork initialization fails', async () => {
		...
		vi.spyOn(artwork, 'createEarthDayArtwork').mockImplementationOnce(() => {
	...
	it('leaves one scene and one animation loop after cleanup and restart', async () => {
		...
		for (let run = 0; run < 3; run += 1) {
			...
			expect(scene.pending.size).toBe(0)
			if (run < 2) {
				cleanupEffect = await launchEarthDay()
