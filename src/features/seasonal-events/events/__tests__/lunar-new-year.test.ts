...
describe('Lunar New Year scene', () => {
	it('waits for its delayed mount and can be canceled before any scene appears', async () => {
		...
		expect(document.querySelector('[data-lunar-new-year]')).toBeNull()
	...
	it('redraws the static scene on resize and rebuilds artwork only for a new pixel density', async () => {
		const createArtwork = vi.spyOn(artwork, 'createLunarNewYearArtwork')
		const scene = await createScene({ isReducedMotion: true })
		const canvas = document.querySelector('canvas[data-lunar-new-year]')

		expect(canvas).toHaveAttribute('data-lunar-new-year', 'true')
		...
		expect(document.querySelectorAll('[data-lunar-new-year]')).toHaveLength(1)
	...
	it('keeps drifting lanterns upright throughout their motion', async () => {
		const scene = await createScene()
	...
	it('ignores canceled callbacks after pause and resume without starting an extra loop', async () => {
		...
		if (!staleCallback)
			throw new Error('Expected a scheduled Lunar New Year frame')
	...
	it('cleans up roots and any listeners if artwork initialization fails', async () => {
		...
		vi.spyOn(artwork, 'createLunarNewYearArtwork').mockImplementationOnce(
			() => {
	...
	it('leaves one scene and one animation loop after cleanup and restart', async () => {
		...
		for (let run = 0; run < 3; run += 1) {
			...
			expect(scene.pending.size).toBe(0)
			if (run < 2) {
				cleanupEffect = await launchLunarNewYear()
