...
import { launchSummerSolstice } from '../summer-solstice'
import * as artwork from '../summer-solstice-artwork'
...
describe('Summer solstice scene', () => {
	it('waits for its delayed mount and can be canceled before any scene appears', async () => {
		...
		expect(document.querySelector('[data-summer-solstice]')).toBeNull()
	...
	it('redraws the static scene on resize and reuses artwork across size and pixel-density changes', async () => {
		const createArtwork = vi.spyOn(artwork, 'createSummerSolsticeArtwork')
		const scene = await createScene({ isReducedMotion: true })
		const canvas = document.querySelector('canvas[data-summer-solstice]')

		expect(canvas).toHaveAttribute('data-summer-solstice', 'true')
		...
		expect(document.querySelectorAll('[data-summer-solstice]')).toHaveLength(1)
	...
	it('ignores canceled callbacks after pause and resume without starting an extra loop', async () => {
		...
		if (!staleCallback) throw new Error('Expected a scheduled summer frame')
	...
	it('cleans up roots and any listeners if artwork initialization fails', async () => {
		...
		vi.spyOn(artwork, 'createSummerSolsticeArtwork').mockImplementationOnce(
			() => {
	...
	it('leaves one scene and one animation loop after cleanup and restart', async () => {
		...
		for (let run = 0; run < 3; run += 1) {
			expect(document.querySelectorAll('[data-summer-solstice]')).toHaveLength(
				1,
			)
			expect(document.body.querySelectorAll('canvas')).toHaveLength(1)
			expect(scene.pending.size).toBe(1)
			...
			expect(scene.pending.size).toBe(0)
			if (run < 2) {
				cleanupEffect = await launchSummerSolstice()
