...
import { launchChristmasScene } from '../christmas'
...
describe.each([Hemisphere.Northern, Hemisphere.Southern])(
	'Christmas %s scene',
	(hemisphere) => {
		it('keeps a static scene visible on resize and rebuilds cached artwork only when pixel density changes', async () => {
			...
			const scene = await createScene({ hemisphere, isReducedMotion: true })
			const canvas = document.querySelector('canvas[data-christmas]')

			expect(canvas).toHaveAttribute('data-christmas', 'true')
			expect(canvas).toHaveAttribute('aria-hidden', 'true')
			expect(canvas).toHaveAttribute(
				'data-christmas-season',
				hemisphere === Hemisphere.Southern ? 'summer' : 'winter',
			)
			...
			expect(createArtwork).toHaveBeenLastCalledWith({
				dpr: 1,
				hasSnow: hemisphere === Hemisphere.Northern,
			})
		...
		it.each([60, 120])(
			...
			async (refreshRate) => {
				const scene = await createScene({ hemisphere })
		...
		it('keeps one scene and one animation loop after repeated cleanup and restart', async () => {
			...
			for (let run = 0; run < 3; run += 1) {
				...
				expect(scene.pending.size).toBe(0)
				if (run < 2) cleanupEffect = await launchChristmasScene({ hemisphere })
			}
		})
	},
)
...
const createScene = async ({
	...
	cleanupEffect = await launchChristmasScene({ hemisphere })
