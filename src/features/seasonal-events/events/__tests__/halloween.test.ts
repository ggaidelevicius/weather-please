...
describe('Halloween scene', () => {
	...
	it('redraws a visible static scene and reuses its artwork and particles across resize and density changes', async () => {
		...
		const canvas = document.querySelector('canvas[data-halloween]')
		expect(canvas).toHaveAttribute('data-halloween', 'true')
	...
	it('leaves one scene and animation loop after cleanup and restart', async () => {
		...
		for (let run = 0; run < 3; run += 1) {
			expect(document.querySelectorAll('[data-halloween]')).toHaveLength(1)
			expect(document.body.querySelectorAll('canvas')).toHaveLength(1)
			expect(scene.pending.size).toBe(1)
			cleanupEffect()
			expect(document.querySelector('[data-halloween]')).toBeNull()
...
async function createScene({
	...
	const createArtwork = vi
		.spyOn(artwork, 'createHalloweenArtwork')
		.mockImplementation(() => {
	...
	return {
		...
		getPendingCallback: () => {
			...
			if (!callback) throw new Error('Expected a scheduled Halloween frame')
			return callback
		},
		getSnapshot,
		motion,
