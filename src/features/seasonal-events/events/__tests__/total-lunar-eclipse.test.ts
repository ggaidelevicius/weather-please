...
vi.mock('../total-lunar-eclipse-artwork', () => ({
...
beforeEach(() => {
	...
	vi.mocked(createTotalLunarEclipseArtwork).mockReset()
...
describe('Total lunar eclipse scene', () => {
	it('can cancel the delayed mount before allocating artwork or drawing', async () => {
		...
		expect(createTotalLunarEclipseArtwork).not.toHaveBeenCalled()
		expect(document.querySelector('[data-total-lunar-eclipse]')).toBeNull()
		cleanupEffect()
	...
	it('renders a visible static background and preserves artwork and particles across resizing', async () => {
		...
		for (const artworkName of ['moon', 'shade', 'star']) {
			expect(
				scene
					.snapshot()
					.some(
	...
	it('keeps the moon and shadow aligned through animation and resizing while preserving star positions', async () => {
		const scene = await createScene()
		for (let frame = 0; frame <= 100; frame += 1) {
			scene.runFrame(frame * 50)
			if (frame % 25 === 0) expectEclipseAlignment(scene.snapshot())
		}
		const moon = scene.snapshot().find(({ source }) => source === 'moon')
		const shade = scene.snapshot().find(({ source }) => source === 'shade')
		scene.runFrame(5050)
		expect(scene.snapshot().find(({ source }) => source === 'moon')).toEqual(
			moon,
		)
		expect(
			scene.snapshot().find(({ source }) => source === 'shade')?.alpha,
		).not.toBe(shade?.alpha)
	...
	it('cleans up immediately after resume, removing exact listeners and ignoring late callbacks', async () => {
		...
		const stale = scene.getPendingCallback()
		const drawCount = scene.context.clearRect.mock.calls.length
		const artwork: ReturnType<typeof createTotalLunarEclipseArtwork> =
			vi.mocked(createTotalLunarEclipseArtwork).mock.results[0].value
		const canvas = document.querySelector<HTMLCanvasElement>(
			'canvas[data-total-lunar-eclipse]',
		)
		if (!canvas) throw new Error('Missing Total lunar eclipse canvas')
		cleanupEffect()
		cleanupEffect()
		stale(120_000)
		setSettingsModalOpenState(true)
	...
	it.each(['context', 'artwork', 'initial draw'])(
		...
		async (stage) => {
			...
			if (stage === 'context') scene.getContext.mockReturnValueOnce(null)
				...
				vi.mocked(createTotalLunarEclipseArtwork).mockImplementationOnce(() => {
	...
	it('leaves one canvas and one animation loop after cleanup and relaunch', async () => {
		...
		for (let run = 0; run < 3; run += 1) {
			expect(
				document.querySelectorAll('[data-total-lunar-eclipse]'),
			).toHaveLength(1)
			expect(document.body.querySelectorAll('canvas')).toHaveLength(1)
			expect(scene.pending.size).toBe(1)
			cleanupEffect()
			...
			expect(document.querySelector('[data-total-lunar-eclipse]')).toBeNull()
			if (run < 2) {
				cleanupEffect = await launchTotalLunarEclipse()
				vi.advanceTimersByTime(900)
			}
		}
		expect(createTotalLunarEclipseArtwork).toHaveBeenCalledTimes(3)
	})
})
...
function expectEclipseAlignment(images: ImageDraw[]) {
	...
	const shade = images.find(({ source }) => source === 'shade')
	if (!moon || !shade) throw new Error('Missing moon or shadow layer')
	expect(images.indexOf(moon)).toBeLessThan(images.indexOf(shade))
	for (const [x, y] of [
		[0.5, 0.5],
		[0, 0.5],
		[1, 0.5],
		...
		const moonPoint = getImagePoint(moon, x, y)
		const shadePoint = getImagePoint(shade, x, y)
