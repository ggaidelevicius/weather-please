...
describe('Autumn equinox scene', () => {
	it('keeps a static scene visible on resize and rebuilds cached artwork only when pixel density changes', async () => {
		const createArtwork = vi.spyOn(artwork, 'createAutumnArtwork')
		const scene = await createScene({ isReducedMotion: true })
		const canvas = document.querySelector('canvas[data-autumn-equinox]')

		expect(canvas).toHaveAttribute('data-autumn-equinox', 'true')
	...
	it.each(['running', 'queued', 'resumed'])(
		...
		async (frameState) => {
			...
			expect(document.querySelector('[data-autumn-equinox]')).toBeNull()
			expect(scene.pending.size).toBe(0)
	...
	it('keeps one scene and one animation loop after repeated cleanup and restart', async () => {
		...
		for (let run = 0; run < 3; run += 1) {
			expect(document.querySelectorAll('[data-autumn-equinox]')).toHaveLength(1)
			expect(document.body.querySelectorAll('canvas')).toHaveLength(1)
			expect(scene.pending.size).toBe(1)
			cleanupEffect()
			expect(document.querySelectorAll('[data-autumn-equinox]')).toHaveLength(0)
			expect(scene.pending.size).toBe(0)
			if (run < 2) cleanupEffect = await launchAutumnEquinoxLeaves()
		}
	})
})
...
const createScene = async ({
	...
	cleanupEffect = await launchAutumnEquinoxLeaves()
