...
import { launchEidAlAdhaGlow } from '../eid-al-adha'
import { createEidAlAdhaArtwork } from '../eid-al-adha-artwork'

vi.mock('../eid-al-adha-artwork', () => ({ createEidAlAdhaArtwork: vi.fn() }))
...
beforeEach(() => {
	...
	vi.mocked(createEidAlAdhaArtwork).mockReset()
...
describe('Eid al-Adha scene', () => {
	it('can cancel the delayed mount before allocating artwork or drawing', async () => {
		...
		expect(createEidAlAdhaArtwork).not.toHaveBeenCalled()
		expect(document.querySelector('[data-eid-al-adha]')).toBeNull()
		cleanupEffect()
	...
	it('renders a visible static background and preserves artwork and particles across resizing', async () => {
		...
		const canvas = document.querySelector('canvas[data-eid-al-adha]')
		...
		expect(createEidAlAdhaArtwork).toHaveBeenCalledOnce()
		expect(scene.pending.size).toBe(0)
	...
	it('keeps standing lanterns bottom-anchored and stars at normalized positions on resize', async () => {
		const scene = await createScene({ isReducedMotion: true })
		const lanternsBefore = scene
			.snapshot()
			.filter(({ source }) => source.startsWith('lantern'))
		const starsBefore = scene
			.snapshot()
			.filter(({ source }) => source === 'star')
		expect(lanternsBefore.length).toBeGreaterThan(0)
		expect(starsBefore.length).toBeGreaterThan(0)
		vi.stubGlobal('innerHeight', 888)
		window.dispatchEvent(new Event('resize'))
		const lanternsAfter = scene
			.snapshot()
			.filter(({ source }) => source.startsWith('lantern'))
		const starsAfter = scene
			.snapshot()
			.filter(({ source }) => source === 'star')
		const artwork: ReturnType<typeof createEidAlAdhaArtwork> = vi.mocked(
			createEidAlAdhaArtwork,
		).mock.results[0].value
		expect(lanternsAfter).toHaveLength(lanternsBefore.length)
		expect(starsAfter).toHaveLength(starsBefore.length)
		for (const [index, image] of lanternsBefore.entries()) {
			const sprite = artwork.lanterns.find(
				({ canvas }) => canvas.dataset.testSprite === image.source,
			)
			if (!sprite) throw new Error('Missing standing lantern artwork')
			const before = getImagePoint(image, sprite.baseY / sprite.height)
			const after = getImagePoint(
				lanternsAfter[index],
				sprite.baseY / sprite.height,
			)
			expect(after.x).toBeCloseTo(before.x, 8)
			expect(after.y).toBeCloseTo(before.y + 120, 8)
		}
	...
	it('cleans up immediately after resume, removing exact listeners and ignoring late callbacks', async () => {
		...
		const canvas = document.querySelector<HTMLCanvasElement>(
			'canvas[data-eid-al-adha]',
		)
		if (!canvas) throw new Error('Missing Eid al-Adha canvas')
		cleanupEffect()
		cleanupEffect()
		stale(120_000)
		setSettingsModalOpenState(true)
		...
		for (const surface of [
			canvas,
			...artwork.lanterns.map(({ canvas }) => canvas),
	...
	it.each(['context', 'artwork', 'initial draw'])(
		...
		async (stage) => {
			...
			if (stage === 'context') scene.getContext.mockReturnValueOnce(null)
				...
				vi.mocked(createEidAlAdhaArtwork).mockImplementationOnce(() => {
	...
	it('leaves one canvas and one animation loop after cleanup and relaunch', async () => {
		...
		for (let run = 0; run < 3; run += 1) {
			expect(document.querySelectorAll('[data-eid-al-adha]')).toHaveLength(1)
			expect(document.body.querySelectorAll('canvas')).toHaveLength(1)
			expect(scene.pending.size).toBe(1)
			cleanupEffect()
			expect(scene.pending.size).toBe(0)
			expect(document.querySelector('[data-eid-al-adha]')).toBeNull()
			if (run < 2) {
				cleanupEffect = await launchEidAlAdhaGlow()
				vi.advanceTimersByTime(900)
			}
		}
		expect(createEidAlAdhaArtwork).toHaveBeenCalledTimes(3)
	})
})
...
async function createScene({
	...
	vi.mocked(createEidAlAdhaArtwork).mockImplementation(({ dpr }) => ({
	...
	return {
		...
		getPendingCallback: () => {
			...
			if (!callback) throw new Error('Expected a scheduled Eid al-Adha frame')
			return callback
		},
...
function getImagePoint(image: ImageDraw, relativeY = 0.5) {
	let x = image.coordinates[0] + image.coordinates[2] / 2
