...
This document defines code quality standards and guidelines for AI agents
...
2. **Minimal changes.** Only change what's necessary. Don't refactor unrelated
   code, add unnecessary comments, or "improve" things not requested.
3. **Match existing style.** Follow the conventions already established in the
   codebase for formatting, naming, and structure.
4. **Test your changes.** Run `pnpm test` and `pnpm lint` before considering a
   task complete.
